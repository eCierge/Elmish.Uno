...
module GroupedCollectionTarget =
  ...
  //    GetKeys = ct.GetKeys >> Seq.map kOut
  //    Get = kIn >> ct.Get
  //    Add = fun key items -> ct.Add (kIn key) items
  //    Remove = kIn >> ct.Remove
  //    Clear = ct.Clear
  //    GetCollection = ct.GetCollection }
...
module Merge =
  ...
  let keyed
    ...
    while (shouldContinue && curSourceIdx < sourceCount && curTargetIdx < targetCount) do
      ...
      if curSourceId = curTargetId then
        ...
        let mNextTarget =
          if curTargetIdx + 1 < targetCount then target.GetAt (curTargetIdx + 1) |> ValueSome else ValueNone
          |> ValueOption.map (fun t ->
            let id = getTargetId t
            t, id, id = curSourceId) // true => need to remove

        match mNextSource, mNextTarget with
        | ValueSome (nextSource, _, true), ValueSome (nextTarget, _, true) -> // swap adjacent
        ...
        |               ValueNone, ValueSome (nextTarget, _, true)
        | ValueSome (_, _, false), ValueSome (nextTarget, _, true) -> // remove
            ...
            curSourceIdx <- curSourceIdx + 1
            curTargetIdx <- curTargetIdx + 2
        | ValueSome (nextSource, _, true), ValueNone
        | ValueSome (nextSource, _, true), ValueSome (_, _, false) -> // add
            ...
            curSourceIdx <- curSourceIdx + 2
            curTargetIdx <- curTargetIdx + 1
        | ValueSome (_, _, false),               ValueNone
        |               ValueNone, ValueSome (_, _, false)
        |               ValueNone,               ValueNone -> // source and target have different lengths and we have reached the end of one
            shouldContinue <- false
        | ValueSome (nextSource, nextSourceId, false), ValueSome (nextTarget, nextTargetId, false) ->
  ...
  let grouped
    ...
    let removals =
      ...
      removals
    let additions =
      let additions = SortedSet<'key>(sourceKeys, keyComparer)
      additions.ExceptWith targetKeys
      additions
    let updates =
      let updates = SortedSet<'key>(sourceKeys, keyComparer)
      updates.IntersectWith targetKeys
      updates

    removals |> Seq.iter target.Remove
